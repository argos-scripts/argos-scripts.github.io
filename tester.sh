...
case "$rbversion" in
  ...
  ;;
  *)
    echo "${yellow}Sorry, I cant find your Ruby version to set your path${reset}"
    exit
  ;;
esac
...
if [ -d _vendor ]
then
  echo "${green}Found _vendor folder${reset}"
...
case $1 in
  ...
  *)
    echo "${yellow}Browser will not start${reset}"
  ;;
esac
...
echo "${green}Jekyll output can be found at logs/jekyllLog.txt"
echo -e "$(gettext 'Press ENTER to stop Jekyll and leave script')${reset}"
function pause(){
  read -rp "$*"
